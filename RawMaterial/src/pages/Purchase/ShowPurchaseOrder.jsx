...
const ShowPurchaseOrder = () => {
  ...
  const handleReOrder = async (poId) => {
    ...
    try {
      ...
      const response = await Api.get(`/purchase/purchase-orders/details/${poId}`);
  ...
  const handleUpdatePo = async (poData) => {
    ...
    try {
      // Fetch order details first
      const response = await Api.get(`/purchase/purchase-orders/details/${poData.id}`);
      ...
      setShowUpdateForm(true);
      fetchItems(); // Fetch items for dropdown
              ...
              {poListing.length === 0 ? (
                ...
                poListing?.map((po, index) => (
                  <tr key={po.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.vendorName || 'N/A'}</td>
                    <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.items?.length || '0'}</td>
                    <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.expectedDeliveryDate?.split("T")[0] || "N/A"}</td>
                    <td className="px-2 py-2 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(po?.status)}`}>
                        {getStatusLabel(po?.status)}
                      </span>

...
//     });
//     // console.log('Navigating to receive stock with order ID:', selectedOrderDetails);
//   };
...
//                     {/* Conditionally show "Update Order" button only if status is not "Update Order" */}
...
const ShowPurchaseOrder = () => {
  ...
  const handleReOrder = () => {
    ...
    const reorderData = {
      ...
      gstRate: selectedOrderDetails.gstRate,
      currency: selectedOrderDetails.currency,
      items: selectedOrderDetails.items?.map(item => ({
        id: item.id || '',
        ...
        rate: item.rate || '',
        quantity: item.quantity || '1',
        gstRate: item.gstRate || selectedOrderDetails.gstRate,
        itemDetail: item.itemDetail || ''
      })) || [],
      otherCharges: selectedOrderDetails.otherCharges?.map(charge => ({
        name: charge.name || '',
        amount: charge.amount || ''
      })) || []
    };

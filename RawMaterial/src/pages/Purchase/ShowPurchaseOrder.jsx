...
//   const isItemWiseGST = formData.gstType === 'IGST_ITEMWISE' || formData.gstType === 'LGST_ITEMWISE';
...
//       warehouseName: selectedOrderDetails.warehouseName || "",
...
//     if (isItemWiseGST) {
//       const itemsWithoutGstRate = formData.items.filter(item =>
//         !item.gstRate || item.gstRate.trim() === '' || isNaN(parseFloat(item.gstRate))
//       );
...
//       const newIsItemWiseGST = value === 'IGST_ITEMWISE' || value === 'LGST_ITEMWISE';
...
//           quantity: '1',
//           rate: '',
//           gstRate: isItemWiseGST ? '' : undefined, // Only include gstRate for itemwise GST
//           total: ''
//         }
//       ]
//     }));
...
//                       </div>
//                       <div>
//                         <label className="block text-sm font-medium text-gray-600 mb-2">Warehouse Name</label>
//                         <div className="text-base font-semibold text-gray-900">{selectedOrderDetails.warehouseName || 'N/A'}</div>
//                       </div>
//                       <div>
...
//                       {/* Show GST Rate input only for non-itemwise GST */}
//                       {!isItemWiseGST && (
//                         <div>
//                           <label className="block text-sm font-medium text-gray-700 mb-2">
...
//                             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
//                           </svg>
//                           <span className="font-medium text-blue-800">Itemwise GST Selected</span>
//                         </div>
//                         <p className="text-sm text-blue-600 mt-1">
//                           GST rate must be entered for each item individually in the items section below.
//                         </p>
//                       </div>
//                     )}
...
//   // Handle Cancel Purchase Order
//   const handleCancelOrder = async (poNumber) => {
...
//     try {
//       const response = await Api.put(
//         `/purchase/purchase-orders/cancel/${poNumber}`
//       );
...
//     const handleReOrder = (selectedOrderDetails) => {
//     if (!selectedOrderDetails) {
//       return false;
//     }
...
//   // Download as PDF
//   const handleDownload = async (poNumber) => {
//     console.log('Downloading PDF for PO Number:', poNumber);
//     if (!poNumber) return;

//     setDownloadLoading(true);
//     try {
//       const response = await Api.post(
//         `/purchase/purchase-orders/download/${poNumber}`,
//         {},
//         {
//           responseType: 'blob'
//         }
...
//       const fileName = `PurchaseOrder_${poNumber}.pdf`;
//       link.setAttribute('download', fileName);
...
//       if (selectedCompany) queryParams.push(`company=${encodeURIComponent(selectedCompany.split(",")[0])}`);
...
//         console.log("PO Search Response:", response.data);
//         setPoListing(response.data || []);
//       }
//     };
//     useEffect(() => {poSearch()},[])
//     const PO_Table_Heading = ["SR.No.","PO Number", "PO Date", "Company", "Vendor", "Item Count", "expected Delivery", "Status"," Actions"];
//   return (
//     <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
...
//                     <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{++index || '#'}</td>
//                     <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.poNumber || 'N/A'}</td>
//                     <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{formatDate(po.poDate) || 'N/A'}</td>
//                     <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.companyName || 'N/A'}</td>
//                     <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.vendorName || 'N/A'}</td>
//                     <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.items?.length || '0'}</td>
//                     <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{po?.expectedDeliveryDate?.split("T")[0] || "N/A"
// }</td>
...
//       );
//       const selectedOrderDetails = response.data.data;
...
//             parseFloat(selectedOrderDetails.gstRate) ||
//             0;
...
//       vendorName:
//         formData.vendorName ||
//         vendorsList.find((v) => v.id === formData.vendorId)?.displayName ||
//         "",
...
//     console.log("Downloading PDF for PO ID:", poId);
//     if (!poId) return;
...
//                             className="px-5 py-2.5 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all duration-200 font-medium flex items-center justify-center"
//                             onClick={() =>
//                               handleReOrder(selectedOrderDetails.id)
//                             }
//                           >
//                             <svg
//                               className="w-4 h-4 mr-2"
...
//                           className="px-5 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-200 font-medium flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
//                           onClick={() =>
//                             handleDownload(selectedOrderDetails.id)
//                           }
//                           disabled={downloadLoading}
//                         >
//                           {downloadLoading ? (
...
//                       {/* In the basic information display section */}
//                       <div>
//                         <label className="block text-sm font-medium text-gray-600 mb-2">
//                           Vendor Name
//                         </label>
...
//                       {/* In the update form modal, add vendor dropdown field */}
//                       <div>
//                         <label className="block text-sm font-medium text-gray-700 mb-2">
...
//                       {/* In the update form modal, replace the warehouseName input with this: */}
//                       <div>
//                         <label className="block text-sm font-medium text-gray-700 mb-2">
...
//                                       {" "}
//                                       {/* Use unit.name as value */}
//                                       {unit.label}{" "}
//                                       {/* Display unit.name as label */}
//                                     </option>
//                                   ))
//                                 )}
//                               </select>
...
//                                 Current value: {item.unit || "empty"} |
//                                 Available units:{" "}
//                                 {unitTypes.map((u) => u.name).join(", ")}
//                               </div> */}
//                             </div>

...
const CreatePurchaseOrder = () => {
  ...
  useEffect(() => {
    if (location.state?.reorderData) {
      ...
      setSelectedVendor(reorderData.vendorId);
      setSelectedGstType(reorderData.gstType);
      setCurrency(reorderData.currency);
      setExchangeRate(reorderData.exchangeRate || "1.00");
      setPaymentTerms(reorderData.paymentTerms || "");
      setDeliveryTerms(reorderData.deliveryTerms || "");
      setWarranty(reorderData.warranty || "");
  ...
  // --- FIXED updateItemDetail FUNCTION ---
  const updateItemDetail = (id, field, value) => {
    // If clearing a value, mark that we should skip calculation temporarily
    if (
      value === "" &&
    ...
    setItemDetails(
      itemDetails.map((item) => {
        if (item.id === id) {
          ...
          // For fields that affect the total, recalc everything using calculateItemAmounts
          if (
            field === "rate" ||
            field === "quantity" ||
            field === "amount" ||
            (isItemWiseGST && field === "gstRate")
          ) {
            const calculated = calculateItemAmounts(updatedItem);
            return {
              ...updatedItem,
          ...
          // For other fields (modelNumber, itemDetail, etc.) just update
          return updatedItem;
        }
        return item;
      }),
